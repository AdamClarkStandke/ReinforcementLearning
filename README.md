...
The PPO Agent was trained for 50 thousand steps of SPY data ranging from 2005 to mid-2017 and was tested on SPY data ranging from the end-of-2017 to 2022. Furthermore, 4 parallel environments were used for the agent to gather samples/experience to train on. All of the default hyper-parameters from stable-Baselines3 were used except for the number of epochs when optimizing the surrogate loss which I set to 20. I also set the entropy coefficient to 0.01 and used stable-Baselines3's limit implementation regarding the KL divergence between updates and set it to 0.2. After analyzing the training results, explained variance was zero; meaining that basically the best policy generated was not better than random guessing by a monkey lol!!:upside_down_face: But having said that, some lucky random trading actions were generated by the agent (hmm random walk theory??):shrug: :monkey: :dart: in which the Agent finished with 16,000 dollars in its account as can be seen in the video down below :point_down:
...
**Rewards on Validation Set**
![](https://github.com/aCStandke/ReinforcementLearning/blob/main/eval_mean_reward_SortinoRatioRewardTrial1-2_Default.svg)

**Total Loss**
![](https://github.com/aCStandke/ReinforcementLearning/blob/main/train_loss_SortinoRatioRewardTrial1-2_Default.svg)

**Value Loss**
![](https://github.com/aCStandke/ReinforcementLearning/blob/main/train_value_loss_SortinoRatioReward_trial1-2_Default.svg)
